...
//------------------------------------------------------------------------------
// Overlading input stream operator to read data from file
...
// assume format: ( year : young middle old )
{
   ...
   if (is >> ch1 >> dd.year
       ...
       >> ch3) {
      if (ch1!= '(' || ch2!=':' || ch3!=')') {            // checking data format
	 is.clear(ios_base::failbit);                     // return is after setting fail 
	 return is;
      }
   }
   else
...
//------------------------------------------------------------------------------
// Helper class to scale data wrt our plot/graph
...
int main()
{
   try
	 ...
	 const double xscale = double(xlength)/(end_year-base_year); // scale of x values
	 const double yscale = double(ylength)/100;  // scale of y values 

	 Scale xs{xoffset,base_year,xscale};
	 Scale ys{ymax-yoffset,0,-yscale};            // negative yscale as y axis grow downward
